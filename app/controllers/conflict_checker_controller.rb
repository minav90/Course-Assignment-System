  ...
  def create
    ...
    if @building == ""
       ...
       @faculty_preferences.each do |faculty_preference|
         ...
         # Check Pref 1
		 if ( (@pref_1) && (@day_combo_row) && (@time_slot_row) &&(@pref_1.day_combination_id == @day_combo_row.id) && ((findTimeSlotDataFromId((@pref_1.time_slot_id)).time_slot == @time_slot_row.time_slot)))
			@relevant_preferences.insert(i, [@course_name, @course_title, @faculty_name, @pref_1, 1])
			i += 1
		 end
		     # Check Pref 2
		 if ( (@pref_2) && (@day_combo_row) && (@time_slot_row) && (@pref_2.day_combination_id == @day_combo_row.id) && ((findTimeSlotDataFromId((@pref_2.time_slot_id)).time_slot == @time_slot_row.time_slot)))
			@relevant_preferences.insert(i, Array[@course_name, @course_title, @faculty_name, @pref_2, 2])
			i += 1
		 end
		     # Check Pref 3
		 if ( (@pref_3) && (@day_combo_row) && (@time_slot_row) && (@pref_3.day_combination_id == @day_combo_row.id) && ((findTimeSlotDataFromId((@pref_3.time_slot_id)).time_slot == @time_slot_row.time_slot)))
       ...
       @relevant_preferences.each do |relevant_preference|
		...
		# Already Assigned
		@already_asgn = isAssigned((relevant_preference[3]).building_id, (relevant_preference[3]).day_combination_id, (relevant_preference[3]).time_slot_id) # building_id, day_combination_id, time_slot_id
	...
	j += 1
       ...
       @faculty_preferences.each do |faculty_preference|
         ...
         # @pref_1 = Preference.find_by preference_id: @pref1_id
			 if ( (@pref_1) && (@day_combo_row) && (@time_slot_row) && (@building_row) && (@pref_1.day_combination_id == @day_combo_row.id) && ((findTimeSlotDataFromId((@pref_1.time_slot_id)).time_slot == @time_slot_row.time_slot)) && (@pref_1.building_id == @building_row.id) )
				@relevant_preferences.insert(i, [@course_name, @course_title, @faculty_name, @pref_1, 1])
				i += 1
			 end
				 # Check Pref 2
			 if ( (@pref_2) && (@day_combo_row) && (@time_slot_row) && (@building_row) && (@pref_2.day_combination_id == @day_combo_row.id) && ((findTimeSlotDataFromId((@pref_2.time_slot_id)).time_slot == @time_slot_row.time_slot)) && (@pref_2.building_id == @building_row.id) )
				@relevant_preferences.insert(i, Array[@course_name, @course_title, @faculty_name, @pref_2, 2])
				i += 1
			 end
				 # Check Pref 3
			 if ( (@pref_3) && (@day_combo_row) && (@time_slot_row) && (@building_row) && (@pref_3.day_combination_id == @day_combo_row.id) && ((findTimeSlotDataFromId((@pref_3.time_slot_id)).time_slot == @time_slot_row.time_slot)) && (@pref_3.building_id == @building_row.id) )
				@relevant_preferences.insert(i, Array[@course_name, @course_title, @faculty_name, @pref_3, 3])
				i += 1
			 end  
