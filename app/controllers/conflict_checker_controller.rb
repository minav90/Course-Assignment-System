...
include ConflictCheckerHelper
  ...
  def create  	
    ...
    relevant_preferences = Array.new # Array to store the relevant (as per the inputs) preferences: Course Name, Course Title, Faculty name, preference, preference #
    i = 0
  ...
  # method to store the user inputs in the session
  ...
  # method to get the conflicts from relevant preferences
  def getConflictsFromRelevantPreferences(relevant_preferences)
	...
	j = 0
  	...
  	relevant_preferences.each do |relevant_preference|
		...
		# Variable to check course has been already assigned
